...
class TestGeneration extends Component {
    ...
    render() {
        ...
        const { curriculums, selectedCurriculum } = this.state;

        return (

            <div className="overflow-auto md:mx-auto p-10 px-72 border border-gray-300 rounded bg-ivory">

                {
                    curriculums.length > 0 ?
                        curriculums.map(curriculum => (
                            <button key={curriculum.name} value={curriculum.id} onClick={this.handleCurriculum} className="px-4 py-2 bg-blue-500 mx-4 text-white rounded-lg">{curriculum.name}</button>
                        ))
                        :
